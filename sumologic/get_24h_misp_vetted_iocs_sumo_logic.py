...
def format_iocs_for_sumo(misp_iocs: list[dict]) -> list[dict]:
    ...
    for ioc in misp_iocs:
        ...
        sumo_indicator = {
            "value": ioc["value"],
            "active": True,
            "expiration": str(expiration.strftime("%Y-%m-%dT%H:%M:%SZ")),
            "description": f"RHISAC IOC: Title: {ioc['Event']['info']} - {ioc['comment']} - MISP Event ID: {ioc['Event']['id']} - Tags: {' | '.join([tag['name'] for tag in ioc['Tag']])}",
        }
...
def send_iocs_to_sumo(iocs: list[dict]):
    ...
    url = f"{sumo_endpoint}/sec/v1/threat-intel-sourffces/{threatIntelSourceId}/items"
...
if __name__ == "__main__":
    print(
        f"Starting RHISAC IOC to Sumologic Script for {datetime.now().strftime('%Y-%m-%dT%H:%M')}"
    )
    # Get IOCs from MISP
