...
def can_access_splunk(splunk_creds: dict) -> bool:
    ...
    try:
        test_resp = requests.get(url, headers=SPLUNK_HEADERS, verify=False)
    except Exception as e:
        print(
            f"Unable to connect to Splunk. Check the base_url in your rh-isac.conf file: {str(e)}"
        )
        return False

    if test_resp.status_code == 401:
        print(
            "Failed to authenticate to Splunk. Verify the token in the rh-isac.conf file is correct and has the proper access"
        )
        return False
...
def dispatch_lookup_gen_searches() -> bool:
    ...
    for name in search_names:
        ...
        try:
            resp = requests.post(url, headers=SPLUNK_HEADERS, verify=False)
        except Exception as e:
        ...
        if resp.status_code != 201:
            print(
                f"Unexpected response to dispatch of {name} saved search: {resp.status_code}"
            )
            all_success = False
    return all_success
...
def post_iocs_to_splunk(lookup_name: str, iocs: List[dict]) -> bool:
    ...
    try:
        resp = requests.post(url, headers=SPLUNK_HEADERS, data=body, verify=False)
    except Exception as e:
        print(f"Failed to connect to Splunk: {str(e)}")
        return False
...
def categorize(inds: List[dict]) -> Dict[str, List[dict]]:
    ...
    for ind in inds:
        ...
        if type(ind["tags"]) == str:
            fields["description"] = ind["tags"]
        else:
            fields["description"] = " | ".join([t["name"] for t in ind["tags"]])

        if ind["type"] in ("ip-dst", "ip-src"):
        ...
        elif ind["type"] == "URL" or ind["type"] == "url":
            value = ind["value"]
...
def get_last24h_vetted_iocs() -> List[dict]:
    ...
    # Query API for IOCs
    try:
        results = misp.search("attributes", tags=[VETTED_TAG], timestamp="14d")
...
if __name__ == "__main__":
    ...
    #    print(
    #        f'Missing config section "{SPLUNK_CONFIG_SECTION}". Please check the example configuration and try again.'
    #    )
    #    exit()
    ...
    if have_splunk_access:
        for lookup_name, iocs in ioc_cats.items():
            if not iocs:
                ...
                if result:
                    print(
                        f"Successfully uploaded {len(iocs)} IOCs to lookup {lookup_name}"
                    )
                else:
